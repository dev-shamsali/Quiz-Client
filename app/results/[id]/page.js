...
import { Trophy, CheckCircle2, XCircle, Brain, RotateCcw, ChevronDown, ChevronUp } from 'lucide-react';
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="card mb-6 overflow-hidden">
          <div className="bg-ink p-6 sm:p-10 text-center">
            ...
            <p className="text-cream/40 text-sm mt-2">{attempt.correctAnswers} of {attempt.totalQuestions} correct · {Math.floor(attempt.timeTaken / 60)}m {attempt.timeTaken % 60}s</p>
          </div>
          <div className="card-body">
            <h3 className="font-bold mb-4">Breakdown by Difficulty</h3>
            <div className="grid grid-cols-3 gap-4">
              {Object.entries(attempt.breakdown || {}).map(([diff, data]) => (
                <div key={diff} className={`p-4 text-center border-2 ${diff === 'easy' ? 'border-green-700 bg-green-50' : diff === 'moderate' ? 'border-yellow-700 bg-yellow-50' : 'border-red-700 bg-red-50'}`}>
                  <p className="text-2xl font-black">{data.correct}/{data.total}</p>
                  <p className="text-sm font-semibold capitalize mt-1">{diff}</p>
                  <p className="text-xs opacity-60 mt-0.5">{data.total ? Math.round((data.correct / data.total) * 100) : 0}%</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="card-body">
            {!report && !generating && (
              <div className="text-center py-6">
                ...
                <p className="text-sm text-ink-muted mb-5">Score 1–10, expert feedback & personalised learning roadmap — powered by Gemini AI</p>
              ...
              <div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 sm:p-5 border-2 border-ink bg-cream-dark mb-5 gap-3 sm:gap-0">
                  <div>
                    ...
                    <div className="h-2 bg-ink/10 w-full sm:w-48 mt-2">
                      <motion.div className="h-full bg-ink" initial={{ width: 0 }} animate={{ width: `${(report.analysis.score / 10) * 100}%` }} transition={{ duration: 1 }} />
                    </div>
                  </div>
                  <span className="text-4xl sm:text-5xl font-black">{report.analysis.score}<span className="text-xl sm:text-2xl text-ink-muted font-normal">/10</span></span>
                </div>
                <div className="grid md:grid-cols-2 gap-4 mb-4">
                  <div className="p-4 border border-green-700 bg-green-50">
                    <h4 className="font-bold text-green-900 mb-2 text-sm"><CheckCircle2 size={13} className="inline mr-1" />Strengths</h4>
                    {report.analysis.strengths?.map((s, i) => <p key={i} className="text-xs text-green-800 mb-1 leading-relaxed">• {s}</p>)}
                  </div>
                  <div className="p-4 border border-red-700 bg-red-50">
                    <h4 className="font-bold text-red-900 mb-2 text-sm"><XCircle size={13} className="inline mr-1" />Weaknesses</h4>
                    {report.analysis.weaknesses?.map((w, i) => <p key={i} className="text-xs text-red-800 mb-1 leading-relaxed">• {w}</p>)}
                  </div>
                </div>
                ...
                {report.analysis.areasToImprove?.length > 0 && (
                  <div className="p-4 border border-ink/20 mb-4">
                    ...
                    {report.analysis.areasToImprove.map((a, i) => <p key={i} className="text-xs text-ink-muted mb-1">• {a}</p>)}
                  </div>
                )}
                <div className="p-4 border border-ink/20 bg-cream-dark">
                  <h4 className="font-bold text-sm mb-3">📚 Learning Roadmap</h4>
                  <ol className="space-y-1.5">{report.analysis.learningRoadmap?.map((step, i) => (
                    <li key={i} className="text-xs text-ink-muted flex gap-2"><span className="font-black">{i + 1}.</span>{step}</li>
                  ))}</ol>
                </div>
              </div>
            )}
          ...
          {attempt.questions?.map((q, i) => (
            <div key={i} className="px-6 py-4 border-b border-ink/10 last:border-0">
              <div className="flex items-start gap-3">
                <div className={`mt-0.5 flex-shrink-0 ${q.isCorrect ? 'text-green-700' : 'text-red-700'}`}>
                  {q.isCorrect ? <CheckCircle2 size={17} /> : <XCircle size={17} />}
                </div>
                <div className="flex-1">
                  ...
                  {q.question?.codeSnippet && <pre className="code-block text-xs mb-2">{q.question.codeSnippet}</pre>}
                  <div className="flex flex-wrap gap-2 mb-2">
                    {q.selectedAnswer
                      ? <span className={`px-2 py-0.5 border text-xs ${q.isCorrect ? 'border-green-700 text-green-800 bg-green-50' : 'border-red-700 text-red-800 bg-red-50'}`}>Your answer: {q.selectedAnswer}</span>
                      : <span className="px-2 py-0.5 border border-gray-400 text-gray-600 bg-gray-50 text-xs">Unanswered</span>}
                    {!q.isCorrect && <span className="px-2 py-0.5 border border-green-700 text-green-800 bg-green-50 text-xs">Correct: {q.correctAnswer}</span>}
                  </div>
                  <button onClick={() => setExpanded(p => ({ ...p, [i]: !p[i] }))}
                    className="text-xs font-bold text-ink-muted hover:text-ink flex items-center gap-1">
                    {expanded[i] ? <><ChevronUp size={11} />Hide</> : <><ChevronDown size={11} />Explanation</>}
                  </button>
                  {expanded[i] && <p className="text-xs text-ink-muted mt-2 p-3 bg-cream-dark border border-ink/10 leading-relaxed">{q.question?.explanation}</p>}
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="flex gap-4">
          <Link href="/quiz" className="btn-primary flex-1 py-3 text-center"><RotateCcw size={13} className="inline mr-2" />Retake Quiz</Link>
